...
/**
 * VoltageButton
 * Accessible CTA with SVG voltage lines and gentle glow.
 *
 * Props:
 * - href?: string (renders anchor if provided)
 * - onClick?: function
 * - children: node (button label)
 * - className?: string
 */
function VoltageButton({ href, onClick, children, className = '' }) {
  ...
  return (
    <div className={`voltage-button ${className}`}>
      <Element className="voltage-cta" {...commonProps}>
        {children}
      </Element>
      <svg
